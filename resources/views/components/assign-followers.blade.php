@php
    ...
    $followerNamesJson = $existingFollowers->map(function($follower) {
    ...
    <div>
        ...
        <div id="selected-followers" class="mt-2 flex flex-wrap gap-2">
            @foreach($existingFollowers as $follower)
                <div class="follower-tag bg-blue-100 text-blue-800 text-sm font-medium px-2.5 py-0.5 rounded flex items-center" data-id="{{ $follower->id }}">
                    {{ $follower->name }}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateSelectedFollowers() {
        ...
        selectedFollowers.forEach(id => {
            ...
            tag.className = 'follower-tag bg-blue-100 text-blue-800 text-sm font-medium px-2.5 py-0.5 rounded flex items-center';
            tag.dataset.id = id;
            tag.innerHTML = `
